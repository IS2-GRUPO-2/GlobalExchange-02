...
type MetodoPago = "transferencia" | "billetera" | "tarjeta";

interface EtapaPagoProps {
  tipo: MetodoPago;
  montoOrigen: number;
  divisaOrigen: string;
  detalleClienteSeleccionado: CuentaBancaria | BilleteraDigital | Tarjeta | null;
  onPagar: () => void;
  onCancelar: () => void;
  pagando: boolean;
}

const filtrarActivos = <T extends { metodo_financiero_detalle?: { is_active?: boolean } | number; is_active?: boolean }>(
  items: T[],
) => {
  ...
  const renderDetalleMetodo = () => {
    if (tipo === "transferencia") {
      if (cargandoInstancias) {
        return <p className="text-sm text-gray-600">Cargando cuentas bancarias...</p>;
      }
      if (cuentasCasa.length === 0) {
        return <p className="text-sm text-gray-600">No hay cuentas bancarias activas configuradas para la casa.</p>;
      }
      return (
        <div className="space-y-3">
          {cuentasCasa.map((cuenta) => (
            <div key={cuenta.id} className="rounded-lg border border-gray-200 p-4 bg-white shadow-sm">
              <p className="text-sm text-gray-700">
                <span className="font-medium text-gray-900">Banco:</span> {cuenta.banco_nombre ?? "No especificado"}
              </p>
              <p className="text-sm text-gray-700">
                <span className="font-medium text-gray-900">Número de cuenta:</span>{" "}
                {cuenta.numero_cuenta ? `**** ${cuenta.numero_cuenta.slice(-4)}` : "No disponible"}
              </p>
              <p className="text-sm text-gray-700">
                <span className="font-medium text-gray-900">Titular:</span> {cuenta.titular ?? "No especificado"}
              </p>
              <p className="text-sm text-gray-700">
                <span className="font-medium text-gray-900">CBU/ CVU:</span> {cuenta.cbu_cvu ?? "No especificado"}
              </p>
            </div>
          ))}
        </div>
    ...
    if (tipo === "billetera") {
      if (cargandoInstancias) {
        return <p className="text-sm text-gray-600">Cargando billeteras digitales...</p>;
      }
      if (billeterasCasa.length === 0) {
        return <p className="text-sm text-gray-600">No hay billeteras digitales activas configuradas para la casa.</p>;
      }
      return (
        <div className="space-y-3">
          {billeterasCasa.map((billetera) => (
            <div key={billetera.id} className="rounded-lg border border-gray-200 p-4 bg-white shadow-sm">
              <p className="text-sm text-gray-700">
              ...
              <p className="text-sm text-gray-700">
                ...
                {billetera.usuario_id ?? billetera.alias_billetera ?? "No disponible"}
              </p>
              {billetera.email && (
                <p className="text-sm text-gray-700">
                  <span className="font-medium text-gray-900">Email:</span> {billetera.email}
                </p>
              )}
              {billetera.telefono && (
                <p className="text-sm text-gray-700">
                  <span className="font-medium text-gray-900">Teléfono:</span> {billetera.telefono}
                </p>
              )}
            </div>
          ))}
    ...
    if (!tarjeta) {
      return <p className="text-sm text-gray-600">No se encontró la información de la tarjeta seleccionada.</p>;
    }
    return (
      <div className="rounded-lg border border-gray-200 p-5 bg-white shadow-sm space-y-2">
        <p className="text-sm text-gray-700">
          <span className="font-medium text-gray-900">Marca:</span> {tarjeta.marca_nombre ?? tarjeta.brand ?? "N/D"}
        </p>
        <p className="text-sm text-gray-700">
        ...
        <p className="text-sm text-gray-700">
          <span className="font-medium text-gray-900">Titular:</span> {tarjeta.titular ?? "No especificado"}
        </p>
        <p className="text-sm text-gray-700">
          <span className="font-medium text-gray-900">Vencimiento:</span>{" "}
          {tarjeta.exp_month && tarjeta.exp_year ? `${tarjeta.exp_month}/${tarjeta.exp_year}` : "No disponible"}
        </p>
      </div>
    );
  };
