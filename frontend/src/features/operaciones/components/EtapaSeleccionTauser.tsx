...
export default function EtapaSeleccionTauser({
  ...
  const handleRetroceder = () => onRetroceder?.();
  const handleAvanzar = () => onAvanzar?.();
  const handleCancelar = () => onCancelar?.();
    ...
    const fetchTausers = async () => {
      ...
      try {
        if (opPerspectivaCasa === "compra") {
          ...
          const tauserActivos = Array.isArray(data)
            ? data.filter((t: Tauser) => t.is_active)
            : [];
  ...
  const baseClasses = `space-y-6 select-none ${
