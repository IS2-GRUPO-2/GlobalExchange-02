...
export const getDetallesMetodosFinancieros = async (
  ...
  try {
    ...
    console.error(
      "Error fetching detalles métodos financieros: ",
      err.response?.data || err.message
    );
    throw err;
  }
...
export const updateDetalleMetodoFinanciero = async (
  detalleData: MetodoFinancieroDetalle,
  id: number
) => {
  try {
    const res = await apiClient.put<MetodoFinancieroDetalle>(
      `${API_URL}detalles/${id}/`,
      detalleData
    );
    return res;
...
export const getTarjetas = async (
  ...
  try {
    const res = await apiClient.get<PaginatedTarjeta>(`${API_URL}tarjetas/`, {
      params,
    });
    return res.data;
  } catch (err: any) {
...
export const getBancos = async (
  ...
  try {
    const res = await apiClient.get<PaginatedBanco>(`${API_URL}bancos/`, {
      params,
    });
    return res.data;
  } catch (err: any) {
...
export const updateBanco = async (bancoData: Banco, id: number) => {
  try {
    const res = await apiClient.put<Banco>(
      `${API_URL}bancos/${id}/`,
      bancoData
    );
    return res;
