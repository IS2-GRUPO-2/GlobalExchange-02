...
const MetodosFinancierosClientePage = () => {
  ...
  const fetchAllData = async () => {
    console.log("fetch all data llamado");
    try {
      // Fetch todos los datos SIN parámetros de búsqueda
      const [metodosRes, cuentasRes, billeterasRes, tarjetasRes] =
        await Promise.all([
          getMetodosFinancieros(),
  ...
  const getFilteredItems = (
    ...
    const extendedItems = items.map((item) => ({
      ...
      desactivado_por_catalogo:
        item.metodo_financiero_detalle?.desactivado_por_catalogo ?? false,
    }));
  ...
  const handleCreateItem = async (formData: any) => {
    ...
    try {
      ...
      const getAliasBase = (tab: TabType) => {
        switch (tab) {
          case "cuentas":
            return "Mi cuenta bancaria";
          case "billeteras digitales":
            return "Mi billetera digital";
          case "tarjetas":
            return "Mi tarjeta";
          default:
            return "Mi método financiero";
        }
      };
  ...
  const handleUpdateItem = async (formData: any) => {
    ...
    try {
      ...
      toast.success(
        `${selectedItem.tipo.slice(0, -1)} actualizado exitosamente!`
      );
      fetchAllData();
      closeEditModal();
    } catch (err) {
  ...
  const handleToggleActive = async (item: ExtendedItem) => {
    ...
    try {
      ...
      toast.success(
        `${item.tipo.slice(0, -1)} ${
          item.is_active ? "desactivado" : "activado"
        } exitosamente!`
      );
      fetchAllData();
    } catch (err) {
  ...
  const getMetodoFinancieroId = (tipo: TabType): number => {
    ...
    if (!metodo) {
      ...
      switch (tipo) {
        case "cuentas":
          return 1; // TRANSFERENCIA_BANCARIA
        case "billeteras digitales":
          return 2; // BILLETERA_DIGITAL
        case "tarjetas":
          return 3; // TARJETA
        default:
          return 1;
      ...
      case "tarjetas":
        return (
          <TarjetaForm
            onSubmit={editModalOpen ? handleUpdateItem : handleCreateItem}
            initialData={
              initialData && activeTab === "tarjetas"
                ? (initialData as unknown as Tarjeta)
                : undefined
            }
            isSubmitting={isSubmitting}
          />
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-gray-900">
                  {(tarjeta as any).brand || (tarjeta as any).marca_nombre}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">
              ...
              <div>
                ...
                <p className="text-gray-900">
                  {(tarjeta as any).exp_month?.toString().padStart(2, "0")}/
                  {(tarjeta as any).exp_year}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">
      ...
      <div className="mb-6">
        <div className="border-b border-gray-200">
          <nav className="-mb-px flex space-x-8">
            {(["cuentas", "billeteras digitales", "tarjetas"] as TabType[]).map(
              (tab) => (
                <button
                  key={tab}
      ...
      <div className="card">
        {currentItems.length === 0 ? (
          <div className="text-center py-8">
            <div className="text-gray-400 mb-4">{getTabIcon(activeTab)}</div>
